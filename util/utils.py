...
def fn_getfnameonly(full_filepath):
    spliters = full_filepath.split(os.path.sep)
    fnameext = spliters[-1]
    #print(fnameext)
    fname = fnameext.split('.')[-2]
    #print(fname)
    return fname


def fn_savepath(path_dataset):
    # spliters = path_dataset.split('/')
    # svpath = os.path.join('/', *spliters[:-1], 'out')
    svpath = os.path.join(path_dataset, 'out')
